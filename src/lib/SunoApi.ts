...
// Removed static imports to prevent build-time issues
...
let BrowserContext: any, Page: any, Locator: any, chromium: any, firefox: any;
let createCursor: any, Cursor: any;
let fs: any, path: any;
...
if (typeof window === 'undefined' && process.env.NODE_ENV !== 'development') {
  try {
    ...
    const ghostCursor = require('ghost-cursor-playwright');
    ...
    // Fallback for build-time - create mock objects
    console.warn('Browser dependencies not available during build');
  }
}
            ...
            try {
              ...
              if (drag) {
                ...
                try {
                  ...
                  logger.error('Failed to read drag-instructions.jpg:', error);
                  // Continue without the image instructions if file is not found
                }
              }
